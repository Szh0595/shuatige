...
import com.szh.shuatige.model.dto.question.QuestionEsDTO;
...
public class QuestionServiceImpl extends ServiceImpl<QuestionMapper, Question> implements QuestionService {
    ...
    public void batchDeleteQuestions(List<Long> questionIdList) {
        ...
        for (Long questionId : questionIdList) {
            ...
            //查询是否有关联的题库
            LambdaQueryWrapper<QuestionBankQuestion> wrapper = new LambdaQueryWrapper<QuestionBankQuestion>().eq(QuestionBankQuestion::getQuestionId, questionId);
            long count = questionBankQuestionService.count(wrapper);
            //删除题目题库关联
            if (count > 0) {
                result = questionBankQuestionService.remove(wrapper);
                ThrowUtils.throwIf(!result, ErrorCode.OPERATION_ERROR, "题目题库关联删除失败");
            }
        }
